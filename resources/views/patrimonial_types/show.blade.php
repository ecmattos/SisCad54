...
@section('content')
	...
	<div class="page-header text-primary">
          ...
          Administração - Tipos de Ativos: Consulta
          <div class="btn-group btn-group-sm pull-right">
            ...
            <a href="{!! route('patrimonial_types.edit', ['id' => $patrimonial_type->id]) !!}" type="button" class="round round-sm hollow blue" rel="tooltip" title="Editar"><i class="fa fa-edit"></i></a>
    	...
    	<div class="col-sm-8">
      		<div class="table-responsive">
      			<table class="table table-bordered table-striped">
          			<thead>
           				<th class="text-center" colspan="2"><h3><b>{{ $patrimonial_type->code }} - {{ $patrimonial_type->description }}</b></h3></th>
          			</thead>
          			<tbody>
          				<tr>
           					<td class="text-right" width="25%">Código</td>
           					<td class="text-left">{{ $patrimonial_type->code }}</td>
            			</tr>

        				<tr>
             				<td class="text-right">Descrição</td>
             				<td class="text-left">{{ $patrimonial_type->description }}</td>
           				</tr>
          			</tbody>
        		</table>
      		</div>
